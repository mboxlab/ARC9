...
local LerpAngle = function(a, v1, v2)
	...
	local v22 = v2[2]
	local v23 = v2[3]
	local d1 = math_AngleDifference(v21, v11)
	local d2 = math_AngleDifference(v22, v12)
	local d3 = math_AngleDifference(v23, v13)
...
local math_Clamp = function(a,b,c)
...
function SWEP:GetViewModelPosition(pos, ang)
	...
	offsetpos:Set(self.ActivePos)
	offsetang:Set(self.ActiveAng)
	local maxspd, vel = owner:GetWalkSpeed() or 250, owner:OnGround() and owner:GetAbsVelocity():Length() or 0
	local movingpv = math_Clamp(math.Remap(vel, 0, maxspd, 0, 1), 0, 1)
	...
	if (mvpos or mvang) and movingpv > 0.125 then
		...
		local wim = self:GetValue("MovingMidPoint")
	...
	if getbipod then
		...
		if bipodpos and bipodang then
			LerpVectorEdit(math_Clamp(bipodamount - self:GetSightAmount(), 0, 1), pos, self:GetBipodPos())
		...
		local crouchdelta = math_Clamp(
			math.ease.InOutSine(
				(viewOffsetZ - owner:GetCurrentViewOffset().z) / (viewOffsetZ - owner:GetViewOffsetDucked().z)
			),
			0,
			1
		)
	...
	if reloading then
		...
		if reloadpos then
			if not self:GetProcessedValue("ShotgunReload", true) then
				fuckingreloadprocess = math_Clamp(
					1 - (self:GetReloadFinishTime() - curTime) / (self.ReloadTime * self:GetAnimationTime("reload")),
					0,
					1
				)
	...
	if sightdelta > 0 then
		...
		local fuckingreloadprocess = math_Clamp(
			1 - (self:GetReloadFinishTime() - curTime) / (self.ReloadTime * self:GetAnimationTime("reload")),
			0,
			1
		)
		...
		if fuckingreloadprocess < (reloadanim.PeekProgress or reloadanim.MinProgress or 0.9) then
			if self.PeekPosReloading then
				peekp = "PeekPosReloading"
			end
			if self.PeekAngReloading then
				peeka = "PeekAngReloading"
			end
		end
	...
	if nearwalldelta > 0 then
		...
		-- sprintdelta = math.max(sprintdelta, ts_sprintdelta)
		local sprpos = self:GetProcessedValue("NearWallPos", true)
			or self:GetProcessedValue("SprintPos", true)
			or self:GetProcessedValue("RestPos", true)
		local sprang = self:GetProcessedValue("NearWallAng", true)
			or self:GetProcessedValue("SprintAng", true)
			or self:GetProcessedValue("RestAng", true)
		-- sprpos = LerpVector(ts_sprintdelta, sprpos, self:GetProcessedValue("TraversalSprintPos"))
		-- sprang = LerpAngle(ts_sprintdelta, sprang, self:GetProcessedValue("TraversalSprintAng"))
	...
	if not self:GetProcessedValue("NoViewBob", true) then
	...
	if curvedcustomizedelta > 0 then
		if not self.CustomizeNoRotate then
			...
			local rap_pos, rap_ang =
				self:RotateAroundPoint2(pos, ang, self.CustomizeRotateAnchor, vector_origin, rotateAroundAngle)
			pos:Set(rap_pos)
			ang:Set(rap_ang)
		end
	end
...
function SWEP:GetViewModelFOV()
	...
	local target = (self:GetProcessedValue("ViewModelFOVBase", true) or ownerfov)
		+ (self:GetCustomize() and 0 or convarfov)
	...
	if self:GetInSights() then
		...
		if self.Peeking then
			target = math.max(target, self.PeekMaxFOV or 37)
		end -- low vm fov sights look weird in peek, ez fix
	end
	...
	self.SmoothedViewModelFOV =
		math.Approach(self.SmoothedViewModelFOV, target, math.max(diff / self.AimDownSightsTime, diff, 1) * FrameTime())
